...
async function loadMyListingsFromSupabase(ownerCode) {
  if (!ownerCode) return;
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    const _myListCols = 'id,owner_code,title,suburb,city,ward,type,role,category,price,bedrooms,whatsapp,call_phone,amenities,for_who,nearest_university,students_per_room,hourly_rate,day_rate,day_from,day_to,night_rate,night_from,night_to,weekly_rate,photos,photo,views,created_at,description,status';
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/properties?owner_code=eq.${encodeURIComponent(ownerCode)}&order=created_at.desc&select=${_myListCols}`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    if (!res.ok) { console.warn('Supabase myListings error:', res.status, res.statusText); return; }
...
async function loadListingsFromSupabase() {
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    // Select only columns needed for browse cards — avoids fetching unused fields and reduces payload ~60%
    const _listCols = 'id,owner_code,title,suburb,city,ward,type,role,category,price,bedrooms,whatsapp,call_phone,amenities,for_who,nearest_university,students_per_room,hourly_rate,day_rate,day_from,day_to,night_rate,night_from,night_to,weekly_rate,photos,photo,views,created_at,description';
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/properties?status=eq.active&order=created_at.desc&select=${_listCols}`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    if (!res.ok) { console.warn('Supabase load error:', res.status, res.statusText); return; }
    const data = await res.json();
    if (!data || !data.length) return;
...
async function loadLodgesFromSupabase() {
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    // Select only columns needed for lodge cards — reduces payload significantly vs select=*
    const _lodgeCols = 'id,owner_code,name,location,type,price_per_night,hourly_rate,day_rate,weekly_rate,stars,chalets,max_guests,description,whatsapp,call_phone,features,available,photos,photo,views,created_at';
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/lodges?status=eq.active&order=created_at.desc&select=${_lodgeCols}`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    if (!res.ok) { console.warn('Supabase lodge load error:', res.status, res.statusText); return; }
    const data = await res.json();
    if (!data || !data.length) return;
    ...
    console.warn('Supabase lodge load failed:', e);
...
async function loadMyLodgesFromSupabase(ownerCode) {
  ...
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    const _myLodgeCols = 'id,owner_code,name,location,type,price_per_night,hourly_rate,day_rate,weekly_rate,stars,chalets,max_guests,description,whatsapp,call_phone,features,available,photos,photo,views,created_at,status';
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/lodges?owner_code=eq.${encodeURIComponent(ownerCode)}&order=created_at.desc&select=${_myLodgeCols}`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    if (!res.ok) { console.warn('My lodges load error:', res.status, res.statusText); return; }
    const data = await res.json();
    if (!data || !data.length) return;
    const remoteOwn = data.map(row => ({
      id:            row.id,
    ...
    if (typeof renderDashboard === 'function') renderDashboard(null, myLodges);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Restore locally saved lodges FIRST (instant, no network) ──
  // and render the lodge grid right away from cache so it loads instantly
  try {
  ...
  // Restore public lodges cache
  try {
    const _lc = localStorage.getItem('rc_lodges_cache');
    if (_lc) {
      const { data: _lcd, ts: _lct } = JSON.parse(_lc);
      if (_lcd && _lcd.length && (Date.now() - _lct) < 5 * 60 * 1000) {
        const _lexIds = new Set(lodges.filter(l=>l._dbId).map(l=>l._dbId));
        _lcd.forEach(r => { if (!_lexIds.has(r._dbId)) lodges.push(r); });
      }
    }
  } catch(e) {}
  // Paint lodge grid immediately from cache — replaces skeletons or fills with real data
  const _dGrid = document.getElementById('lodgeGrid');
  if (_dGrid) {
    if (lodges.length) {
      if (typeof filterLodges === 'function') filterLodges();
    } else {
      _dGrid.innerHTML = Array(6).fill(rcSkeletonCard()).join('');
    }
  }
  ...
  // Restore locally saved listings (instant, no network needed)
  try {
...
const Auth = (() => {
  ...
  async function login() {
    ...
    try {
      ...
      applyNavState();
      toast('✓ Logged in!');
      loadMyListingsFromSupabase(code);
      loadMyLodgesFromSupabase(code);
      renderDashboard(myListings.filter(l => l._ownerCode === code));
      showPage('pg-dashboard');
    } catch (err) {
  ...
  async function loadDashboard() {
    ...
    // ── INSTANT PAINT: render from in-memory cache first (zero network delay) ──
    const _cachedProps  = (typeof myListings !== 'undefined' ? myListings : []).filter(l => l._ownerCode === s.code);
    const _cachedLodges = (typeof myLodges   !== 'undefined' ? myLodges   : []).filter(l => l._ownerCode === s.code);
    if (_cachedProps.length || _cachedLodges.length) {
      renderDashboard(_cachedProps, _cachedLodges);
    } else {
    ...
    // ── Fetch properties and lodges in parallel — only select columns we actually need ──
    // Avoids the DataCloneError bug in Supabase JS v2 SDK and cuts load time vs sequential fetches.
    // Selecting specific columns instead of * reduces payload by ~60%.
    const PROP_COLS  = 'id,owner_code,title,suburb,city,ward,type,role,category,price,bedrooms,whatsapp,call_phone,amenities,photos,photo,status,views,created_at,description,hourly_rate,day_rate,weekly_rate';
    const LODGE_COLS = 'id,owner_code,name,location,type,price_per_night,hourly_rate,day_rate,weekly_rate,stars,chalets,max_guests,description,whatsapp,call_phone,features,available,photos,photo,views,created_at';

    let propItems = [];
    let lodgeItems = [];

    const [propRes, lodgeRes] = await Promise.all([
      fetch(
        `${SUPABASE_URL}/rest/v1/properties?owner_code=eq.${encodeURIComponent(s.code)}&select=${PROP_COLS}`,
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
      ).catch(e => { console.warn('Dashboard property fetch failed:', e); return null; }),
      fetch(
        `${SUPABASE_URL}/rest/v1/lodges?owner_code=eq.${encodeURIComponent(s.code)}&select=${LODGE_COLS}`,
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
      ).catch(e => { console.warn('Dashboard lodge fetch failed:', e); return null; })
    ]);

    try {
      if (propRes && propRes.ok) {
        const remoteProps = await propRes.json();
        if (remoteProps && remoteProps.length) {
          ...
          // Sync view counts and photo into in-memory arrays without duplicating
    ...
    try {
      if (lodgeRes && lodgeRes.ok) {
        const remoteLodges = await lodgeRes.json();
        if (remoteLodges && remoteLodges.length) {
          ...
          // Sync view counts into in-memory arrays without duplicating
    ...
    renderDashboard(propItems, lodgeItems);
  }
  ...
  function _startDashPoll() {
    ...
    _dashPollInterval = setInterval(() => {
      ...
      if (dashPage && dashPage.classList.contains('active') && session()) {
        loadDashboard();
      } else {
        _stopDashPoll();
      }
    }, 60000); // refresh every 60 seconds while dashboard is open
  }
  ...
  return { showTab, signup, login, logout, copyCred, copyAll, loadDashboard, renderDashboard, editListing, deleteListing, deleteLodge, onListProperty, continueAfterSignup, goToListProperty, init, _editingId: null, _session: session };
})();
