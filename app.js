...
async function loadMyListingsFromSupabase(ownerCode) {
  ...
  try {
    ...
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/properties?owner_code=eq.${encodeURIComponent(ownerCode)}&order=created_at.desc&select=*`,
...
async function loadListingsFromSupabase() {
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/properties?status=eq.active&order=created_at.desc&select=*`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
    ...
    // Cache for next visit (5 min TTL) — mirrors rc_lodges_cache pattern
    try { localStorage.setItem('rc_listings_cache', JSON.stringify({ data: remoteListings, ts: Date.now() })); } catch(e) {}
...
async function loadLodgesFromSupabase() {
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/lodges?status=eq.active&order=created_at.desc&select=*`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
...
async function loadMyLodgesFromSupabase(ownerCode) {
  ...
  try {
    // Use raw fetch to avoid DataCloneError (Headers object postMessage bug in Supabase JS v2)
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/lodges?owner_code=eq.${encodeURIComponent(ownerCode)}&order=created_at.desc&select=*`,
      { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    );
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore public listings cache instantly — same 5 min TTL pattern as lodges
  try {
    const _pc = localStorage.getItem('rc_listings_cache');
    if (_pc) {
      const { data: _pcd, ts: _pct } = JSON.parse(_pc);
      if (_pcd && _pcd.length && (Date.now() - _pct) < 5 * 60 * 1000) {
        const _pexIds = new Set(listings.filter(l=>l._dbId).map(l=>l._dbId));
        _pcd.forEach(r => { if (!_pexIds.has(r._dbId)) listings.push(r); });
      }
    }
  } catch(e) {}

  // ── Render from cache immediately (before network) so grids show on repeat visits ──
  if (typeof filterListings === 'function' && listings.length) filterListings();
  if (typeof filterLodges === 'function' && lodges.length) filterLodges();
...
const Auth = (() => {
  ...
  async function loadDashboard() {
    ...
    if (_dashContainer) {
      // Instantly render from dashboard cache (stale-while-revalidate)
      try {
        const _dc = localStorage.getItem('rc_dashboard_cache_' + s.code);
        if (_dc) {
          const { propItems: _dcp, lodgeItems: _dcl } = JSON.parse(_dc);
          if ((_dcp && _dcp.length) || (_dcl && _dcl.length)) {
            renderDashboard(_dcp || [], _dcl || []);
          }
        }
      } catch(e) {}
      // Only show skeleton if container is still empty (no cache)
      if (!_dashContainer.innerHTML.trim() || _dashContainer.innerHTML.includes('rc-shimmer')) {
        const _dSkel = `
    ...
    const [propRes, lodgeRes] = await Promise.all([
      fetch(
        `${SUPABASE_URL}/rest/v1/properties?owner_code=eq.${encodeURIComponent(s.code)}&select=*`,
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
      ).catch(e => { console.warn('Dashboard property fetch failed:', e); return null; }),
      fetch(
        `${SUPABASE_URL}/rest/v1/lodges?owner_code=eq.${encodeURIComponent(s.code)}&select=*`,
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } }
    ...
    // Cache dashboard data for instant render on next visit (stale-while-revalidate)
    try {
      if (propItems.length || lodgeItems.length) {
        localStorage.setItem('rc_dashboard_cache_' + s.code, JSON.stringify({ propItems, lodgeItems, ts: Date.now() }));
      }
    } catch(e) {}
  }
