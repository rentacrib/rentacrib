...
// ── Fetch + parse JSON, with a retry on genuine failure ──
// Previously this aborted the request after 8s (headers OR body), which was
// cutting off responses that were just slow, not actually broken — Supabase
// would come back fine given more time. No artificial time limit now; a
// request only fails if the browser itself reports a network/HTTP error, in
// which case we retry once on a fresh connection.
async function fetchJsonWithTimeout(url, options = {}, { retries = 1, label = 'fetch' } = {}) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      console.warn(`⏱️ [${label}] attempt ${attempt + 1} failed after ${(performance.now() - _t0).toFixed(0)}ms${attempt < retries ? ' — retrying on a fresh connection' : ' — no retries left'}:`, e);
  ...
  async function loadDashboard() {
    ...
    const [propResult, lodgeResult] = await Promise.all([
      fetchJsonWithTimeout(
        ...
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } },
        { retries: 1, label: 'dashboard-props' }
      ),
      fetchJsonWithTimeout(
        ...
        { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } },
        { retries: 1, label: 'dashboard-lodges' }
      )
    ]);
