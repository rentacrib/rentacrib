...
async function loadLodgesFromSupabase() {
  try {
    ...
    if (!res.ok) { console.warn('Lodge load error:', res.status); return; }
    const data = await res.json();
    if (!data || !data.length) return;
    ...
    console.warn('Lodge load failed:', e);
    const _eg = document.getElementById('lodgeGrid');
    if (_eg && _eg.innerHTML.includes('Loading lodges')) {
...
async function loadMyLodgesFromSupabase(ownerCode) {
  ...
  try {
    ...
    if (!res.ok) { console.warn('My lodges load error:', res.status); return; }
    const data = await res.json();
    if (!data || !data.length) return;
    const remoteOwn = data.map(row => ({
      id:            row.id,
...
window.addEventListener('DOMContentLoaded', () => {
  // ── Show loading spinners immediately — grids are empty until Supabase responds ──
  const _spinStyle = "grid-column:1/-1;text-align:center;padding:56px 20px;color:var(--text-secondary);font-family:'DM Sans',sans-serif;font-size:14px;";
  const _lGrid = document.getElementById('listingsGrid');
  if (_lGrid) _lGrid.innerHTML = `<div style="${_spinStyle}"><div style="font-size:32px;margin-bottom:12px;">⏳</div>Loading listings…</div>`;
  const _dGrid = document.getElementById('lodgeGrid');
  if (_dGrid) _dGrid.innerHTML = `<div style="${_spinStyle}"><div style="font-size:32px;margin-bottom:12px;">⏳</div>Loading lodges…</div>`;
  ...
  Promise.all([loadListingsFromSupabase(), loadLodgesFromSupabase()]).then(async () => {
...
const Auth = (() => {
  ...
  async function loadDashboard() {
    ...
    // ── Fetch fresh data from Supabase and render directly ──
    // We do NOT push into myListings/myLodges here to avoid accumulating duplicates
    // on repeated calls (60s poll, manual refresh, etc.).
    // The global arrays are managed by loadMyListingsFromSupabase / loadMyLodgesFromSupabase.
    let propItems = [];
    let lodgeItems = [];

    try {
      const { data: remoteProps, error: propErr } = await db
        .from('properties')
        .select('*')
        .eq('owner_code', s.code);
      if (!propErr && remoteProps) {
        propItems = remoteProps
          .filter(row => row.role !== 'lodge' && row.category !== 'lodge') // lodges belong in lodgeItems only
          .map(row => {
    ...
    } catch(e) { console.warn('Dashboard property fetch failed:', e); }

    try {
      const { data: remoteLodges, error: lodgeErr } = await db
        .from('lodges')
        .select('*')
        .eq('owner_code', s.code);
      if (!lodgeErr && remoteLodges) {
    ...
    } catch(e) { console.warn('Dashboard lodge fetch failed:', e); }
