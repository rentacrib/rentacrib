...
async function loadMyLodgesFromSupabase(ownerCode) {
  ...
  try {
    ...
    // Re-render lodge browse grid immediately so owner lodges appear
    // without waiting for Promise.all to resolve (mirrors loadLodgesFromSupabase behaviour)
    if (typeof filterLodges === 'function') filterLodges();
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // TTL check removed — ghost purge after network fetch handles stale entries,
  // so the cache always restores immediately for a faster initial render.
  try {
  ...
  // (previously used a blind 4s timeout which could fire before data arrived)
  {
    ...
    try {
      ...
      // Always re-render lodge grid after purge — not just when ghosts were found.
      // Previously filterLodges() was only called when ghosts existed, meaning the
      // final authoritative grid render was blocked until purge logic completed.
      if (typeof filterLodges === 'function') filterLodges();
...
const Auth = (() => {
  ...
  function init() {
    ...
    // If already logged in, instantly pre-render dashboard from localStorage cache
    // so the user sees real content immediately — network fetch then overwrites with fresh data.
    const s = session();
    if (s) {
      const cachedProps  = (typeof myListings !== 'undefined') ? myListings.filter(l => l._ownerCode === s.code) : [];
      const cachedLodges = (typeof myLodges   !== 'undefined') ? myLodges.filter(l => l._ownerCode === s.code)   : [];
      if (cachedProps.length || cachedLodges.length) {
        renderDashboard(cachedProps, cachedLodges);
      }
      // Network fetch runs in background and refreshes with authoritative data
      loadDashboard();
    }
  }
