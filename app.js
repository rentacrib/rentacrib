...
async function loadLodgesFromSupabase() {
  ...
  try {
    ...
    console.log(`⏱️ [lodges] network response received after ${(_t1 - _t0).toFixed(0)}ms (status ${res.status})`);
    if (!res.ok) { console.warn('Supabase lodge load error:', res.status, res.statusText); return; }
    const data = await res.json();
    const _t2 = performance.now();
    console.log(`⏱️ [lodges] JSON parsed after ${(_t2 - _t1).toFixed(0)}ms — ${data ? data.length : 0} row(s)`);
    if (!data || !data.length) return;
